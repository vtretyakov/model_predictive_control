 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      fg[1 + psi_start + t] = psi1 - (psi0 + v0 * (-delta0) / Lf * dt);//-delta0 is due to inverted angle in the simulator
      fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
      fg[1 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
      fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) + v0 * (-delta0) / Lf * dt);//-delta0 is due to inverted angle in the simulator
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_vars = N * 6 + (N - 1) * 2;//N * size_of_state_vector + (N - 1) * num_of_actuations; (actuations are steering & acceleration)
  // TODO: Set the number of constraints
  size_t n_constraints = N * 6; // N * size_of_state_vector
